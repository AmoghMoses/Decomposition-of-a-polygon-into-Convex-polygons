/**
 * @file Polygon.hpp
 * @brief Defines the Polygon class used for geometric computations on the polygon.
 */
...
class Polygon{
    public : 
        ...
        void merging(DCEL &dcel, vector<pair<deque<int>, pair<int, int>>> decomp);
        ...
        vector<pair<deque<int>, pair<int, int>>> polygon_decomposition(DCEL &dcel, int start);
...
*/
bool Polygon::check_notch(Point *p, Point *q, Point *r) // O(1) time
{
    double a = p->x;
    double b = p->y;
    double c = q->x;
...
bool Polygon::check(pair<int, int> d, deque<int> L)
{
...
*/
void Polygon::merging(DCEL &dcel, vector<pair<deque<int>, pair<int, int>>> decomp) // O(N^3)
{
    int flag_outer = 0;
    do 
    {
...
*/
void Polygon::build_polygon(DCEL &dcel, const vector<Point *> &polygon) // O(V)
{
...
*/
bool Polygon::check_inside_polygon(Point *p, deque<int> v, DCEL &dcel) // O(V)
{
    double flag = -1;
...
*/
double Polygon::helper(double x1, double y1, double x2, double y2, double x, double y) // O(1)
{
...
*/
deque<int> Polygon::semiplane_reduction(int v, deque<int> L, DCEL &dcel) // O(V)
{
    int v1 = L.front();
    deque<int> L1;
...
*/
vector<pair<deque<int>, pair<int, int>>> Polygon::polygon_decomposition(DCEL &dcel, int start) // O(V^2)
{
